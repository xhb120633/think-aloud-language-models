...
This directory contains its own `jspsych/` package (the teaching video is withheld in this review copy); it can be copied independently of Experiment 2. From this directory, run `python -m http.server 8000 --bind 127.0.0.1`, then open http://127.0.0.1:8000/ . Microphone access is needed for the recording portions. The instruction video is a study demonstration, not a participant recording. Do not deploy this reconstruction for recruitment with the historical consent text; a new deployment requires its own approved consent and storage arrangements.
